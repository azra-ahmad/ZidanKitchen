...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($username) < 3 || strlen($username) > 50 || !preg_match('/^[a-zA-Z0-9_]+$/', $username)) {
        $_SESSION['error'] = "Username harus 3-50 karakter dan hanya boleh huruf, angka, atau underscore!";
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "Admin berhasil ditambahkan! Anda akan diarahkan ke halaman login dalam 3 detik.";
        // Redirect otomatis setelah 3 detik
        header("Refresh: 3; url=login.php");
        // Redirect langsung (buat browser yang ga support header Refresh)
        header("Location: login.php");
        exit;
    } else {
        $_SESSION['error'] = "Gagal menambahkan admin: " . $conn->error;
        header("Location: register_admin.php");
        exit;
    }
}
...
<body class="relative flex items-center justify-center h-screen bg-cover bg-center bg-no-repeat bg-[url('../assets/images/bg.png')]">
    ...
    <div class="relative bg-gradient-to-br from-white/90 to-gray-100/80 p-8 rounded-lg shadow-2xl w-full max-w-md backdrop-blur-lg border border-white/30">
        ...
        <?php if (isset($_SESSION['error'])): ?>
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mb-4"><?php echo htmlspecialchars($_SESSION['error']); unset($_SESSION['error']); ?></div>
        <?php endif; ?>
        <?php if (isset($_SESSION['success'])): ?>
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded mb-4"><?php echo htmlspecialchars($_SESSION['success']); unset($_SESSION['success']); ?></div>
        <?php endif; ?>
        ...
        <form action="register_admin.php" method="POST">
            ...
            <div class="floating-label mb-4">
                <input type="password" name="confirm_password" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" placeholder=" " required>
